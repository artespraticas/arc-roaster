...
.ic{background:rgba(255,255,255,.028);border:1px solid var(--br);border-radius:18px;padding:28px;margin-bottom:24px}
...
.ls{font-size:13px;color:var(--dm);margin-top:6px}
...
export default function Home() {
  ...
  function copy() {
    ...
    if (el) navigator.clipboard.writeText(el.innerText).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); });
  }

  const v    = data ? verdict(data.txCount, data.usdcBalance) : '';
      ...
      <main className="pg">
        ...
        {busy && (
          <div className="lb">
            ...
            <div className="ls">Fetching live Arc Testnet data + writing savage roast</div>
            <div className="ls" style={{marginTop:'6px',fontSize:'12px'}}>This takes about 10–15 seconds</div>
          </div>
        )}
        ...
        {data && !busy && (
          <>
            ...
            <div className="ac">
              ...
              <button className="ab" onClick={() => { setAddr(''); setData(null); setErr(''); }}>🔄 Roast Another</button>
            </div>
          </>
        )}
